...
def main():
    ...
    with requests.Session() as s:
        ...
        for item in site_names:
            ...
            if num >= 2:
                for i in range(num-1):
                    if i == 0:
                        dt = Sites[item]['decommissioned_at'] - Sites[site_names[item][i]]['commissioned_at']
                        if dt.days <= 0:
                            merge_site[item].append(item)
                            merge_site[item].append(site_names[item][i])
                    else:
                        dt = Sites[site_names[item][i]]['decommissioned_at'] - Sites[site_names[item][i+1]]['commissioned_at']
                        if dt.days <= 0:
        ...
        for item in merge_site:
            ...
            Sites[merge_site[item][0]]['decommissioned_at'] = Sites[merge_site[item][-1]]['decommissioned_at']
        ...
        datetime_stop = datetime.datetime.strptime(installation_stop, '%m/%d/%y %H:%M:%S')
        ...
        for site in Sites:
            if Sites[site]['status'] == 'in_operation':
                ...
                if dt.days >= 10:
                    print(f'{site}: latest record {latest_time} on {latest_record.housing} is {dt.days} days from now, {dt_stop.days} days from installation stop {datetime_stop}')
                    if  dt_stop.days >= 7:
                        Sites[site]['status'] = 'decommisioned'
                        Sites[site]['decommissioned_at'] = latest_time
        ...
        for site in Sites:
            lst_records.append((set([record.link for record in Sites[site]['records']]), site))
